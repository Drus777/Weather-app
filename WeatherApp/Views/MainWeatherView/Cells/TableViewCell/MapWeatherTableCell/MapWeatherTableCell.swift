...
class MapWeatherTableCell: UITableViewCell {
    
    static let identifier = "MapWeatherTableCell"
    
    var cellModel: MapWeatherTableCellModel?
...
//MARK: fillable
extension MapWeatherTableCell: FillableCell {
    
    
    func fill(by cellModel: CellModel) {
        guard let cellModel = cellModel as? MapWeatherTableCellModel else { return }
        LocationService.shared.getCityName { [ weak self] (placemark, error) in
            ...
            self.annotation.subtitle = cellModel.cityName
...
extension MapWeatherTableCell: CLLocationManagerDelegate {
